  ...
  tags$head(
    ...
    tags$style(HTML("
      ...
      * { box-sizing: border-box; }
      ...
      .badge-comp { display: inline-block; background: #dce8f7; color: #004E9E; border-radius: 12px; padding: 12px 18px; font-size: 1.4rem; line-height: 1.7; }
...
server <- function(input, output, session) {
  ...
  output$ui_comp_badge <- renderUI({
    ...
    lbl_text <- if(is_de())
      tagList(
        tags$b(paste0("Vergleichsstichprobe: N\u00a0=\u00a0", n, " Personen")), tags$br(),
        "Geschlecht: 57,7\u00a0% weiblich, 41,6\u00a0% m\u00e4nnlich, 0,4\u00a0% non-bin\u00e4r", tags$br(),
        "Alter: 18\u201383 Jahre, \u00d8\u00a040,3 Jahre"
      )
    else
      tagList(
        tags$b(paste0("Comparison sample: N\u00a0=\u00a0", n, " participants")), tags$br(),
        "Gender: 52.1\u00a0% male, 47.5\u00a0% female, 0.4\u00a0% non-binary", tags$br(),
        "Age: 18\u201394 years, average: 42.2 years"
      )
  ...
  output$ui_pct_table <- renderUI({
    if(is_de()) {
      ...
      hdr <- c("Prozentrang", "Statistische Einordnung")
    } else {
      rows <- list(
      ...
      hdr <- c("Percentile rank", "Statistical Classification")
    }
  ...
  output$ui_footer <- renderUI({
    url_lab    <- "https://www.psychologie.uni-frankfurt.de/50042693/Willkommen_bei_der_Allgemeinen_Psychologie_II__br_Prof__Dr__Sabine_Windmann"
    url_github <- "https://github.com/luciebinder/fab-app/"
    if(is_de()) {
      tagList(
        tags$span("© 2026 Sabine Windmann & Lucie Binder · "),
        tags$a("ACT Labor, Goethe-Universität Frankfurt", href = url_lab, target = "_blank", class = "footer-link"),
        tags$span(" · "),
        tags$a(
          tags$svg(
          xmlns = "http://www.w3.org/2000/svg", viewBox = "0 0 16 16",
          width = "20", height = "20",
          style = "vertical-align: middle; opacity: 0.6; fill: currentColor;",
          tags$path(d = "M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.013 8.013 0 0016 8c0-4.42-3.58-8-8-8z")
        ),
          href = url_github, target = "_blank", title = "GitHub Repository"
        ),
      ...
      tagList(
        tags$span("© 2026 Sabine Windmann & Lucie Binder · "),
        tags$a("ACT Lab, Goethe University Frankfurt", href = url_lab, target = "_blank", class = "footer-link"),
        tags$span(" · "),
        tags$a(
          tags$svg(
          xmlns = "http://www.w3.org/2000/svg", viewBox = "0 0 16 16",
